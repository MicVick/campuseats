// POST /api/vendor/orders/[id]/preparing — Mark order as preparing
...
export const POST = withErrorHandler(async (request: NextRequest, context) => {
  ...
  if (order.status !== 'accepted') {
    return errorResponse(`Cannot mark as preparing from "${order.status}" status`, 400);
  }
  ...
  const updated = await prisma.order.update({
    ...
    data: {
      status: 'preparing',
      statusTimeline: addStatusToTimeline(order.statusTimeline, 'preparing'),
    },
  });
